...
check_health "http://172.26.224.1:8080" "API Gateway"
check_health "http://172.26.224.1:8081" "Log Producer"
check_health "http://172.26.224.1:8082" "Log Consumer"

# Test 2: Submit log event and verify it's processed
echo "Test 2: End-to-End Log Processing"
test_log_id=$(uuidgen)
response=$(curl -s -X POST http://172.26.224.1:8080/api/logs \
...
if curl -f http://172.26.224.1:8080/actuator/prometheus > /dev/null 2>&1; then
  echo "✅ API Gateway metrics available"
else
  echo "❌ API Gateway metrics not available"
fi

if curl -f http://172.26.224.1:8081/actuator/prometheus > /dev/null 2>&1; then
  echo "✅ Log Producer metrics available"
else
  echo "❌ Log Producer metrics not available"
fi

if curl -f http://172.26.224.1:8082/actuator/prometheus > /dev/null 2>&1; then
